 ...
 * @author	Stephane F
 **/
class plxmobile extends plxPlugin {
	...
	/**
	 * Méthode pour le hook plxMotorConstruct
	 *
	 * @return	stdio
	 * @author	Stéphane F.
	 **/
	public function plxMotorConstruct() {
		...
		if($this->mobileDetect()) {
			if(is_dir(PLX_ROOT.'themes/'.$this->getParam('style_mobile'))) {
				echo "<?php \$this->style = '".$this->getParam('style_mobile')."' ?>";
			}
		}
	}
	 ...
	 */
	private function mobileDetect($iphone=true,$ipad=true,$android=true,$opera=true,$blackberry=true,$palm=true,$windows=true,$mobileredirect=false,$desktopredirect=false){
